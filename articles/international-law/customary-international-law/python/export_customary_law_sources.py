#!/usr/bin/env python3
"""
Export Customary International Law metadata into Markdown for editorial review.
"""
...
def export_markdown() -> None:
    ...
    concepts = read_csv(ARTICLE_DIR / "data" / "customary_law_concepts.csv")

    lines = ["# Customary International Law — Source Export", ""]
    ...
    lines.extend(["", "## Customary-Law Concepts", ""])
    for row in concepts:
        lines.append(
    ...
    output = OUTPUT_DIR / "customary_law_sources_export.md"
    output.write_text("\n".join(lines) + "\n", encoding="utf-8")
