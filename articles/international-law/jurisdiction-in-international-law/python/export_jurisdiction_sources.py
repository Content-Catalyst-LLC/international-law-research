#!/usr/bin/env python3
"""
Export Jurisdiction in International Law metadata into Markdown for editorial review.
"""
...
def export_markdown() -> None:
    ...
    concepts = read_csv(ARTICLE_DIR / "data" / "jurisdiction_concepts.csv")

    lines = ["# Jurisdiction in International Law — Source Export", ""]
    ...
    lines.extend(["", "## Jurisdiction Concepts", ""])
    for row in concepts:
        lines.append(
    ...
    output = OUTPUT_DIR / "jurisdiction_sources_export.md"
    output.write_text("\n".join(lines) + "\n", encoding="utf-8")
