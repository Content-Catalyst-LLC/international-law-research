#!/usr/bin/env python3
"""
Export State Responsibility metadata into Markdown for editorial review.
"""
...
def export_markdown() -> None:
    ...
    concepts = read_csv(ARTICLE_DIR / "data" / "state_responsibility_concepts.csv")

    lines = ["# State Responsibility — Source Export", ""]
    ...
    lines.extend(["", "## State Responsibility Concepts", ""])
    for row in concepts:
        lines.append(
    ...
    output = OUTPUT_DIR / "state_responsibility_sources_export.md"
    output.write_text("\n".join(lines) + "\n", encoding="utf-8")
