#!/usr/bin/env python3
"""
Export Human Rights in International Law metadata into Markdown for editorial review.
"""
...
def export_markdown() -> None:
    ...
    concepts = read_csv(ARTICLE_DIR / "data" / "human_rights_concepts.csv")

    lines = ["# Human Rights in International Law — Source Export", ""]
    ...
    lines.extend(["", "## Human Rights Concepts", ""])
    for row in concepts:
        lines.append(
    ...
    output = OUTPUT_DIR / "human_rights_sources_export.md"
    output.write_text("\n".join(lines) + "\n", encoding="utf-8")
