#!/usr/bin/env python3
"""
Export Sources of International Law metadata into Markdown for editorial review.
"""
...
def export_markdown() -> None:
    ...
    concepts = read_csv(ARTICLE_DIR / "data" / "source_law_concepts.csv")

    lines = ["# Sources of International Law — Source Export", ""]
    ...
    lines.extend(["", "## Source-Law Concepts", ""])
    for row in concepts:
        lines.append(
            f"- **{row['concept_name']}** ({row['source_category']}) — "
            f"{row['description']}."
        )
    ...
    output = OUTPUT_DIR / "sources_of_international_law_export.md"
    output.write_text("\n".join(lines) + "\n", encoding="utf-8")
